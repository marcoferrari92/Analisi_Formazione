...
if uploaded_file is not None:
    try:
        @st.cache_data
        def load_data(file):
            # Carichiamo il file con il separatore ; usato nei passaggi precedenti
            return pd.read_csv(file, sep=';', encoding='utf-8-sig')

        df_raw = load_data(uploaded_file)
        
        # Pulizia Importi: trasforma stringhe con virgola in numeri decimali
        df_raw['RNA_IMPORTO'] = pd.to_numeric(df_raw['RNA_IMPORTO'].astype(str).str.replace(',', '.'), errors='coerce').fillna(0)
        
        # Elaborazione parole chiave (tutto in maiuscolo per il confronto)
        ...
        # Identificazione righe target nel database grezzo
        ...
        # --- GENERAZIONE REPORT SINTETICO (Un riga per Azienda) ---
        ...
        # --- CALCOLO RANKING (1 = Migliore nel database) ---
        ...
        # --- RICERCA AZIENDA E DETTAGLIO CON RANKING ---
        st.subheader("🎯 Analisi Dettagliata per Azienda")
        search_txt = st.text_input("Inserisci Ragione Sociale per visualizzare storia e posizionamento (Ranking)")
        ...
        if search_txt:
            ...
            if not azienda_details.empty:
                ...
                # Recupero dati sintetici e ranking
                ...
                # Funzione di styling per evidenziare le righe target
                ...
                st.dataframe(
                    ...
                    column_config={
                        "is_target": None, # Nascondiamo la colonna tecnica
...
if uploaded_file is not None:
    try:
        ...
        # --- NUOVA SEZIONE: GRAFICO SCATTER ---
        st.divider()
        st.subheader("📈 Analisi Correlazione: Budget Target vs Budget Totale")
        st.markdown("""
        Questo grafico mostra il posizionamento di ogni azienda. 
        - **Asse X**: Volume totale di aiuti ricevuti.
        - **Asse Y**: Volume di aiuti ricevuti nel settore target (es. Formazione).
        - **Dimensione Punto**: Numero totale di bandi vinti.
        """)
        ...
        st.scatter_chart(
            ...
            y='VALORE_TARGET_€',
            size='N_TOT_AIUTI',
            color='#2ecc71', # Colore verde target
            use_container_width=True
        )
        ...
        st.sidebar.download_button(
            ...
            file_name="Report_RNA_Intelligence.csv",
            mime="text/csv"
        )
    ...
    st.info("👋 Benvenuto! Carica il file CSV arricchito per iniziare l'analisi strategica.")
