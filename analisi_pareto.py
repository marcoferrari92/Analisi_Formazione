...
* :orange[Zona **Arancio (fino al 50%):**] aziende che completano la metà dell'intero budget del Settore Target.
* :yellow[Zona **Gialla (fino al 80%):**] aziende che completano l'80% del Mercato Target (regola di Pareto). 
* :green[Zona **Verde (fino al 95%):**] aziende che completano la quasi totalità del Settore Target.  
* :blue[Zona **Blu (oltre il 95%):**] troviamo i *Disinteressati*: aziende non interessate a investire nel Settore Target o troppo piccole per essere determinanti in questo settore. 
...
def pareto_analysis(df, guida_pareto=""):
    ...
    # 3. Opzionale: Aggiungi un marker nel punto di intersezione sulla diagonale
    fig_pareto.add_trace(go.Scatter(
        x=[punto_teorico_80],
        y=[80],
        mode='markers',
