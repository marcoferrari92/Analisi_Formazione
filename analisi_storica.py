...
def story_analysis(df):
   ...
   with col3:
      st.markdown(r"""**In questa sezione trovi l'*Analisi Storica* che confronta la crescita composta (CAGR) del *Settore Target* con l'andamento degli aiuti erogati negli anni.**""")
   st.write("")
      ...
      if cagr_att > 0 and diff_cagr > 0 and diff_aiuto > 0 and diff_n > 0:
         ...
         with col_info:
            st.markdown(f"""
               ...
               * :yellow[**Analisi:** Il trend storico accelera per inerzia, ma l'anno corrente segna una contrazione reale su tutti i fronti. Verificare la saturazione del mercato.]
               """)
